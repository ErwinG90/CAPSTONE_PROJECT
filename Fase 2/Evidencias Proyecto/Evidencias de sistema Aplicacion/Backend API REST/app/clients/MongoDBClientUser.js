// app/clients/MongoDBClientUser.js
const config = require('../configs/config');
const { MongoClient, ObjectId } = require('mongodb'); // ← agregado ObjectId
...
class MongoDBClientUser {
  ...
  // ✅ quitar id de ruta del usuario (soporta ObjectId y string)
  async pullRuta(uid, rutaId) {
    try {
      if (!this.collection) await this.connect();

      // candidatos: ObjectId (si es válido) y string
      const candidates = [];
      try { candidates.push(new ObjectId(rutaId)); } catch (_) { /* no válido como ObjectId */ }
      candidates.push(rutaId);

      const result = await this.collection.updateOne(
        { _id: uid },
        { $pull: { rutas: { $in: candidates } } }
      );

      console.info(
        `[MongoDBClientUser] pullRuta uid=${uid} rutaId=${rutaId} matched=${result.matchedCount} modified=${result.modifiedCount}`
      );
      return result;
    } catch (e) {
      console.error(`[MongoDBClientUser] pullRuta error:`, e);
      throw e;
    }
  }
}
